...
<head>
    ...
    <title>Sequence Diagram — {{ $system->name }}</title>
...
<div class="seq-app">
    <div class="seq-toolbar">
        <div>
            <h1>Sequence Diagram Designer — {{ $system->name }}</h1>
            <small style="opacity:0.75">Model API and integration message flows</small>
        </div>
        <div class="seq-toolbar-actions">
            <input type="text" id="process-name" class="form-control form-control-sm" placeholder="Process name" style="width:200px">
            <button type="button" id="btn-save" class="seq-btn seq-btn-success">Save Process</button>
            ...
            <a href="{{ route('systems.processes', $system) }}" class="seq-btn seq-btn-secondary">Back</a>
        </div>
    </div>
...
<script>
    window.SequenceDesignerConfig = {
        ...
        systemId: {{ $system->id }},
        initialSource: @json($template),
        storeUrl: @json(route('systems.store.bpmn')),
        backUrl: @json(route('systems.processes', $system)),
    };
</script>
